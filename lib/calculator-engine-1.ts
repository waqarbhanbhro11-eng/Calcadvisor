...
export function getFields(calculator: Calculator): CalculatorField[] {
  ...
  switch (calculator.formulaType) {
    ...
    case "budget": {
      const budgetInputs: string[] = (calculator as any).inputs || [];
      return [
        { name: "income",   label: budgetInputs[0] || "Monthly Income",    type: "number" as const, step: "any", defaultValue: "5000" },
        { name: "fixed",    label: budgetInputs[1] || "Fixed Expenses",    type: "number" as const, step: "any", defaultValue: "1500" },
        { name: "variable", label: budgetInputs[2] || "Variable Expenses", type: "number" as const, step: "any", defaultValue: "800"  },
        { name: "savings",  label: budgetInputs[3] || "Savings Target",    type: "number" as const, step: "any", defaultValue: "500"  },
        { name: "buffers",  label: budgetInputs[4] || "Other Buffers",     type: "number" as const, step: "any", defaultValue: "200"  },
      ];
    }
    default: {
      const autoInputs: string[] = (calculator as any).inputs || [];
...
export function computeCalculator(calculator: Calculator, values: Record<string, string | number>) {
  ...
  switch (calculator.formulaType) {
    ...
    case "budget": {
      ...
      const totalOutflow = fixed + variable + savings + buffers;
    ...
    default: {
      const autoInputs: string[] = (calculator as any).inputs || [];
      const autoOutputs: string[] = (calculator as any).outputs || [];
      if (autoInputs.length > 0) {
        const fieldNames = autoInputs.map((label: string) =>
          label.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "")
        );
        const allVals = fieldNames.map((name: string) => v(name));
        const first = allVals[0] ?? 0;
        const rest  = allVals.slice(1).reduce((a: number, b: number) => a + b, 0);
        const result = first - rest;
        const isMoney = !["health", "math", "conversion"].includes(calculator.categorySlug ?? "");
        const fmt = (n: number) => isMoney ? `$${round(n)}` : round(n);
        return {
          primaryLabel: autoOutputs[0] ?? "Result",
          primaryValue: fmt(result),
          supporting: allVals.slice(1, 4).map((val: number, i: number) => ({
