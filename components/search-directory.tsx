  ...
  return (
    <div className="space-y-14">
      ...
      <section id="search" className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="relative grid gap-10 p-6 sm:p-10 lg:grid-cols-[1.3fr_0.7fr] lg:items-start">
          <div>
            ...
            <p className="mt-4 max-w-xl text-base leading-7 text-slate-500">
              ...
              No ads. No account needed. Just fast, accurate answers.
            </p>
            ...
            {query && (
              <p className="mt-2 text-xs text-slate-500">
                <span className="font-semibold">{filtered.length}</span> result{filtered.length !== 1 ? "s" : ""} for &ldquo;{query}&rdquo;
              </p>
            )}
          </div>
          ...
          <div className="lg:pt-2">
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-2">Quick links</p>
              <div className="space-y-1.5">
                {[
                  { label: "Financial calculators", href: "/categories/financial" },
                  { label: "Mortgage calculators",  href: "/categories/mortgage" },
                  { label: "Health calculators",    href: "/categories/health" },
                  { label: "All 12 categories",     href: "/#categories" },
                ].map(link => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="flex items-center justify-between rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-white hover:shadow-sm"
                  >
                    {link.label}
      ...
      <section id="categories">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-950">Browse by category</h2>
            <p className="mt-1.5 text-sm text-slate-500">12 topic areas · 200+ tools total</p>
          </div>
        </div>
      ...
      <section id="popular">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-950">Featured calculators</h2>
            <p className="mt-1.5 text-sm text-slate-500">Commonly used tools across all categories</p>
          </div>
          <Link
      ...
      {!query && (
        <section>
          <div className="mb-6">
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-950">All calculators</h2>
            <p className="mt-1.5 text-sm text-slate-500">Full directory, grouped by category</p>
          </div>
              ...
              return (
                <div key={category.slug}>
                  <div className="mb-4 flex items-center justify-between gap-3">
                    ...
                    <Link href={`/categories/${category.slug}`} className="text-sm font-medium text-blue-600 hover:text-blue-800 transition">
                      All {tools.length} tools →
                    </Link>
                  </div>
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {tools.slice(0, 6).map(tool => <ToolCard key={tool.slug} tool={tool} />)}
                  </div>
                  {tools.length > 6 && (
                    <Link
                      href={`/categories/${category.slug}`}
                      className="mt-4 flex w-full items-center justify-center rounded-2xl border border-dashed border-slate-300 py-3 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:bg-white hover:text-slate-900"
                    >
      ...
      {query && (
        <section>
          <div className="mb-6">
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-950">Search results</h2>
          ...
          {grouped.map(({ category, tools }) => (
            <div key={category.slug} className="mb-8">
              <h3 className="mb-3 text-base font-bold text-slate-700">{category.name}</h3>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          {grouped.length === 0 && (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center">
              ...
              <p className="text-slate-600 font-medium">No calculators matched &ldquo;{query}&rdquo;</p>
              <button onClick={() => setQuery("")} className="mt-4 text-sm text-blue-600 hover:underline">
                Clear search
              </button>
            </div>
          )}
