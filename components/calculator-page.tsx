    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
        <section className="space-y-4">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">{calculator.title}</h1>
          <p className="max-w-3xl text-base leading-7 text-slate-600">{calculator.intro}</p>

          <div className="flex flex-wrap gap-3">
            <Link href={getCategoryUrl(category.slug)} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
              Explore {category.name}
            </Link>
            <Link href="/" className="rounded-full border border-slate-200 bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-slate-600">
            {calculator.howItWorks.map((step, index) => (
              <p key={index}><span className="font-semibold text-slate-900">Step {index + 1}.</span> {step}</p>
            ))}
          </div>
        </div>
      ...
      <section className="mt-10 space-y-5">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href={getCategoryUrl(category.slug)} className="text-sm font-medium text-slate-600 transition hover:text-slate-950">
      ...
      <section className="mt-10 rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm leading-7 text-slate-600">
        <p>
          Internal links: <Link href="/" className="font-medium text-slate-900 underline underline-offset-4">homepage</Link> ·{" "}
          <Link href={getCategoryUrl(category.slug)} className="font-medium text-slate-900 underline underline-offset-4">{category.name}</Link>
        </p>
      </section>
